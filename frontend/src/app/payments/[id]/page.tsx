...
const PIPELINE: { status: string; label: string; description: string }[] = [
  ...
  },
  {
    status: "VALIDATED",
    label: "Validated",
    description: "Amount, currency, and account details verified",
  },
  {
    status: "RISK_ANALYZED",
    label: "Risk Analysis",
    description: "Transaction scored against risk thresholds",
  },
  {
    status: "FRAUD_CHECKED",
    label: "Fraud Check",
    description: "Fraud detection rules evaluated",
  },
  {
    status: "QUEUED",
    label: "Queued",
    description: "Accepted by processing queue, awaiting worker",
  },
  {
    status: "PROCESSING",
    label: "Processing",
    description: "Worker picked up payment, executing transfer",
  },
  {
    status: "SENT",
    label: "Sent",
    description: "Funds dispatched to destination network",
  },
  {
    status: "SETTLED",
    label: "Settled",
    description: "Payment confirmed and fully settled",
  },
];
    ...
    <div className="mx-auto max-w-5xl space-y-8 px-6 py-10 lg:px-8">
      ...
      <header className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm font-semibold uppercase tracking-wide text-orange-700">
            Payment Detail
          </p>
      ...
      <section className="panel rounded-2xl p-6">
        ...
        <p className="mt-0.5 text-sm text-slate-500">
          Each payment moves through these stages. Fraud and risk rules are
          evaluated at the highlighted steps.
        </p>
              ...
              <li key={step.status} className="flex gap-4">
                ...
                <div className={`pb-6 ${isLast ? "pb-0" : ""}`}>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {(step.status === "RISK_ANALYZED" ||
                      step.status === "FRAUD_CHECKED") && (
                  ...
                  <p
                    className={`mt-0.5 text-xs ${state === "pending" ? "text-slate-300" : "text-slate-500"}`}
                  >
                    {step.description}
                  </p>
      ...
      <section className="panel rounded-2xl p-6">
        ...
        <p className="mt-0.5 mb-4 text-xs text-slate-500">
          All recorded transitions, newest first
        </p>
        {history.length === 0 ? (
          ...
          <div className="space-y-2">
            {[...history].reverse().map((row) => (
              <div
                key={row.id}
                className="flex flex-wrap items-center gap-3 rounded-lg border border-black/5 px-4 py-2.5 text-sm"
              >
